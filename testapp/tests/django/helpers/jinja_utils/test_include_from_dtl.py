...
def test_include_from_dtl_renders_jinja_inside_a_django_template():
    ...
    including_jinja_name = (
        "test_include_from_dtl__including_jinja_basic.jinja2"
    )
    ...
    including_jinja_source = jinja_template_get_source(
        including_jinja_template
    )
    assert expected_jinja_content in including_jinja_source
    assert expected_dtl_content not in including_jinja_source
    assert (
        f'{{{{ include_from_dtl("{included_dtl_name}") }}}}'
        in including_jinja_source
    )

    included_dtl_source = dtl_template_get_source(
        get_template(included_dtl_name)
    )
...
def test_include_from_dtl_passes_context_to_the_rendered_jinja():
    ...
    included_dtl_name = (
        "test_include_from_dtl__included_dtl_context_consuming.html"
    )
