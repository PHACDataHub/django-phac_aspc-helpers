...
class PhacAspcOAuthBackend(BaseBackend):
    """Authentication backend that creates a user using only the id and email"""
    ...
    def authenticate(
        ...
        if user_info is not None:
            ...
            try:
                user = user_model.objects.get(username=user_info["id"])
                self._sync_user(user, user_info)
            except user_model.DoesNotExist:
                user = user_model(username=user_info["id"])
